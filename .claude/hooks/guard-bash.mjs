...
const PORT_BOUND = [
  {
    pattern:
      /\b(?:nx|pnpm)\s+(?:run\s+)?dev(?::|\b)|\bnx\s+start\b|\bnx\s+run\s+\S+:(?:dev|start)\b|\bnx\s+run-many\s+[^\n]*-t\s*[^\s]*\bdev\b|\bnext\s+(?:dev|start)\b/,
    what: 'dev 서버',
  },
  {
    pattern:
      /\b(?:pnpm|nx)\s+(?:run\s+)?e2e\b|\bnx\s+run\s+\S+:e2e\b|\bnx\s+run-many\s+[^\n]*-t\s*[^\s]*e2e/,
    what: 'Playwright e2e',
  },
  {
    pattern: /\b(?:pnpm|nx)\s+(?:run\s+)?storybook(?::a11y)?\b|\bnx\s+run\s+\S+:storybook\b/,
    what: 'Storybook 서버',
  },
];

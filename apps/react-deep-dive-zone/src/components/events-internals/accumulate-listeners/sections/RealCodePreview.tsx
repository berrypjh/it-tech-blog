...
type Props = { content: ListenerCollectionContent['realCode'] };

const KEYWORDS = new Set(['let', 'while', 'if', 'const', 'null', 'return']);
...
const renderToken = (tok: string, i: number) => {
  ...
  if (
    tok === 'getListener' ||
    tok === 'createDispatchListener' ||
    tok === 'accumulateSinglePhaseListeners'
  )
    return (
...
const HIGHLIGHT_KEYWORDS = ['getListener(', 'listeners.push'];

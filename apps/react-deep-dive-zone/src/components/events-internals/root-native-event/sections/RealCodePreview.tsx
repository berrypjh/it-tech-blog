...
type Props = { content: RootNativeEventContent['realCode'] };

const KEYWORDS = new Set(['import', 'from', 'function', 'const', 'return']);
const STRINGS = /^['"`].*['"`]$/;
const COMMENT_PREFIX = /^\/\//;

const isComment = (line: string) => COMMENT_PREFIX.test(line.trim());
...
const renderToken = (tok: string, i: number) => {
  ...
  if (
    tok === 'listenToAllSupportedEvents' ||
    tok === 'createRoot' ||
    tok === 'container' ||
    tok === 'options' ||
    tok === 'rootContainerElement' ||
    tok === 'disableCommentsAsDOMContainers' ||
    tok === 'nodeType' ||
    tok === 'COMMENT_NODE' ||
    tok === 'parentNode'
  )
          ...
          <pre className="overflow-x-auto px-md py-md text-[12px] sm:text-xsm leading-[1.85] font-mono">
            <code>
              {lines.map((line, i) => {
                ...
                const highlight = line.includes('listenToAllSupportedEvents(rootContainerElement)');

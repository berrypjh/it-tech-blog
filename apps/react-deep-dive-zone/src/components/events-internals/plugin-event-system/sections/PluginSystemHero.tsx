...
export const PluginSystemHero = ({ content }: Props) => (
  <section aria-labelledby="hero-heading">
    <TerminalPrompt
      command="cat"
      path="react-dom/events/plugin-event-system.md"
      suffix={
    ...
    <article
      className={cn(
        'mt-md rounded-3xl border-2 p-md sm:p-lg lg:p-xl',
        'border-[var(--term-border)] bg-[var(--term-bg)] shadow-[0_2px_0_var(--term-border)]',
      )}
    >
      {/* Header: section number + heading + description */}
      <header className="flex flex-col gap-sm mb-md sm:mb-lg">
        <span
          aria-hidden="true"
          className="inline-flex h-7 sm:h-8 w-fit items-center justify-center rounded-lg px-2.5 font-mono text-xsm sm:text-sm font-bold tabular-nums tracking-wider bg-blue-100 text-blue-700 dark:bg-blue-950/40 dark:text-blue-200"
        >
        ...
        <h1
          id="hero-heading"
          className={cn(
            'text-2xl sm:text-3xl lg:text-4xl xl:text-[2.4rem]',
            'font-bold leading-[1.18] tracking-tight text-[var(--term-fg)] break-keep',
          )}
        >
          {content.title}
        </h1>
        <p className="text-xsm sm:text-sm md:text-md leading-relaxed text-[var(--term-muted)] break-keep max-w-[80ch]">
          {content.description}
        </p>
      </header>

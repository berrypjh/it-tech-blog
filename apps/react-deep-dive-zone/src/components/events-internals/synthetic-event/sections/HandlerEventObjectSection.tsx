...
const renderToken = (tok: string, i: number) => {
  ...
  if (tok === 'Button' || tok === 'handleClick' || tok === 'console' || tok === 'stopPropagation')
    return (
  ...
  if (tok === 'e')
    return (
      <span key={i} className="text-cyan-700 dark:text-cyan-300 font-bold">
        {tok}
      </span>
    );
  return (
...
export const HandlerEventObjectSection = ({ content }: Props) => {
  const lines = content.code.split('\n');
  return (
    <section aria-labelledby="heading-handler-e">

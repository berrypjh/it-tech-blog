...
const MappingRow = ({ row }: { row: MappingPair }) => (
  ...
  >
    <span className="flex min-w-0 items-center gap-2">
      ...
      <code className={cx('min-w-0 truncate font-mono text-sm font-bold', toneTokens.sky.text)}>
        {row.native}
      </code>
    </span>
    ...
    <code className={cx('min-w-0 truncate font-mono text-sm font-bold', toneTokens.teal.text)}>
      {row.prop}
    </code>
  </article>
);
...
const HelperNote = ({ title, body }: { title: string; body: string }) => (
  ...
  >
    ...
    <div className="flex min-w-0 flex-col gap-1">
      <h3 className={cx('text-xsm font-bold tracking-tight', toneTokens.violet.text)}>{title}</h3>

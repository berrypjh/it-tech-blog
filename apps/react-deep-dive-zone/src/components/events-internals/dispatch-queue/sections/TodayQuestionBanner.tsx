...
import { HelpCircleIcon, ListOrderedIcon, PauseCircleIcon, RepeatIcon } from '../icons';

type Props = { content: DispatchQueueOrderContent['question'] };

const badgeIcons = [RepeatIcon, ListOrderedIcon, PauseCircleIcon];
    ...
    <div className="flex flex-col gap-md lg:flex-row lg:items-center lg:gap-lg">
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-3 lg:flex lg:flex-col gap-2 lg:min-w-[280px]">
        {content.badges.map((label, i) => {
          const Icon = badgeIcons[i] ?? RepeatIcon;
          return (
            <li
              key={label}
              className={cn(

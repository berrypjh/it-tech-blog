...
type Props = { content: DispatchQueueOrderContent['realCode'] };

const KEYWORDS = new Set(['for', 'let', 'const', 'if', 'return']);
...
const renderToken = (tok: string, i: number) => {
  ...
  if (tok === 'processDispatchQueue' || tok === 'executeDispatch' || tok === 'isPropagationStopped')
    return (
  ...
  if (
    ...
    tok === 'dispatchListeners' ||
    tok === 'previousInstance' ||
    tok === 'instance' ||
    tok === 'currentTarget' ||
    tok === 'listener' ||
    tok === 'event'
...
const HIGHLIGHT_KEYWORDS = ['isPropagationStopped()', 'executeDispatch('];
      ...
      <pre className="overflow-x-auto px-md py-md text-[12px] sm:text-xsm leading-[1.85] font-mono">
        <code>
          {lines.map((line, i) => {
            ...
            const isHighlight = !isComment && HIGHLIGHT_KEYWORDS.some((kw) => line.includes(kw));
            return (
              <div
                key={i}
                className={cn(

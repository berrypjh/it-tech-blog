...
type Props = { content: TargetFiberContent['realCode'] };

const KEYWORDS = new Set(['if', 'const', 'let', 'return']);
...
const renderToken = (tok: string, i: number) => {
  ...
  if (
    tok === 'dispatchEventForPluginEventSystem' ||
    tok === 'findInstanceBlockingTarget' ||
    tok === 'return_targetInst' ||
    tok === 'blockedOn'
  )
    return (
...
const HIGHLIGHT_KEYWORDS = ['dispatchEventForPluginEventSystem', 'return_targetInst'];

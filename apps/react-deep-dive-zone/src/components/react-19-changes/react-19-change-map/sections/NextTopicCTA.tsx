...
import { ArrowRightIcon, ExternalLinkIcon, RotateCcwIcon, TelescopeIcon, ZapIcon } from '../icons';

type Props = { content: React19ChangeMapContent['nextCTA'] };
...
const SecondaryButton = ({
  ...
  const Icon = index === 0 ? RotateCcwIcon : TelescopeIcon;
...
export const NextTopicCTA = ({ content }: Props) => (
  <section
    aria-labelledby="next-topic-heading"
    className={cn(
  ...
  >
    ...
    <div
      aria-hidden="true"
      className="absolute -right-20 -top-20 h-64 w-64 rounded-full bg-blue-500/30 blur-3xl pointer-events-none"
    />

    <div className="relative grid grid-cols-1 gap-md lg:gap-lg lg:grid-cols-[auto_minmax(0,_1fr)_minmax(0,_320px)] items-center">
      {/* lightning / Actions visual */}
      <div className="flex justify-center lg:justify-start">
        <span
          aria-hidden="true"
          className={cn(
            'relative inline-flex h-16 w-16 sm:h-20 sm:w-20 items-center justify-center rounded-2xl',
            'border-2 border-amber-300/60 bg-white/10 backdrop-blur-sm shadow-[0_4px_12px_rgba(251,191,36,0.3)]',
          )}
        >
          <ZapIcon className="h-8 w-8 sm:h-10 sm:w-10 text-amber-200" />
        </span>
      </div>

      {/* center */}
      <div className="flex flex-col gap-2 min-w-0">
        <span className="text-[10px] sm:text-xsm font-mono font-bold uppercase tracking-wider text-amber-300">
          {content.eyebrow}
        </span>
        <h2
          id="next-topic-heading"
          className="text-lg sm:text-xl lg:text-xxl font-bold leading-snug text-white break-keep"
        >
          <span className="block text-amber-200/90 text-sm sm:text-md font-mono uppercase tracking-wider">

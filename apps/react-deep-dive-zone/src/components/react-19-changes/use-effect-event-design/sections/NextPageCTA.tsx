...
import { ArrowRightIcon, CompassIcon, ExternalLinkIcon, RotateCcwIcon, TargetIcon } from '../icons';

type Props = { content: UseEffectEventContent['nextCTA'] };
...
export const NextPageCTA = ({ content }: Props) => (
  ...
  >
    ...
    <div
      aria-hidden="true"
      className="absolute -right-20 -top-20 h-64 w-64 rounded-full bg-indigo-400/30 blur-3xl pointer-events-none"
    />
      ...
      <div className="flex justify-center lg:justify-start">
        <span
          ...
          className={cn(
            ...
            'border-2 border-indigo-300/60 bg-white/10 backdrop-blur-sm shadow-[0_4px_12px_rgba(129,140,248,0.3)]',
          )}
        >
          <CompassIcon className="h-8 w-8 sm:h-10 sm:w-10 text-indigo-200" />
        </span>
      </div>

      <div className="flex flex-col gap-2 min-w-0">
        <span className="text-[10px] sm:text-xsm font-mono font-bold uppercase tracking-wider text-indigo-300">
          {content.eyebrow}
        </span>
        <h2
          id="next-cta-heading"
        ...
        >
          <span className="block text-indigo-200/90 text-sm sm:text-md font-mono uppercase tracking-wider">

...
import { ArrowRightIcon, ExternalLinkIcon, RotateCcwIcon, SendIcon, TargetIcon } from '../icons';

type Props = { content: ActionsUpdateFlowContent['nextCTA'] };
...
export const NextPageCTA = ({ content }: Props) => (
  <section
    aria-labelledby="next-page-heading"
    className={cn(
      'relative overflow-hidden rounded-3xl border-2 p-md sm:p-lg lg:p-xl',
      'border-blue-700/60 bg-gradient-to-br from-blue-950 via-blue-900 to-sky-900',
  ...
  >
    ...
    <div
      aria-hidden="true"
      className="absolute -right-20 -top-20 h-64 w-64 rounded-full bg-sky-400/30 blur-3xl pointer-events-none"
    />
      ...
      <div className="flex justify-center lg:justify-start">
        <span
          ...
          className={cn(
            ...
            'border-2 border-sky-300/60 bg-white/10 backdrop-blur-sm shadow-[0_4px_12px_rgba(56,189,248,0.3)]',
          )}
        >
          <SendIcon className="h-8 w-8 sm:h-10 sm:w-10 text-sky-200" />
        </span>
      </div>

      <div className="flex flex-col gap-2 min-w-0">
        <span className="text-[10px] sm:text-xsm font-mono font-bold uppercase tracking-wider text-sky-300">
          {content.eyebrow}
        </span>
        <h2
          id="next-page-heading"
          className="text-lg sm:text-xl lg:text-xxl font-bold leading-snug text-white break-keep"
        >
          <span className="block text-sky-200/90 text-sm sm:text-md font-mono uppercase tracking-wider">

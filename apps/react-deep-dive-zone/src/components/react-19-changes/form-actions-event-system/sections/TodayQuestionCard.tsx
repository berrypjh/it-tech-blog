...
type Props = { content: FormActionsEventSystemContent['question'] };

export const TodayQuestionCard = ({ content }: Props) => {
  const visibleLines = content.questionLines.filter((line) => line.length > 0);
    ...
    <section aria-labelledby="today-question-heading" className="flex flex-col">
      ...
      >
        <div className="grid grid-cols-1 gap-md lg:gap-lg lg:grid-cols-[auto_minmax(0,_5fr)_minmax(0,_7fr)] items-stretch">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2 self-stretch">
            {content.supportQuestions.map((q) => {
              const Icon = iconRegistry[q.iconKey];
              return (
                <li key={q.body} className="h-full">
                  <article
                    className={cn(
                  ...
                  >
                    <span
                      aria-hidden="true"
                      className="mt-0.5 inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border border-blue-200 bg-blue-100 text-blue-700 dark:border-blue-800/60 dark:bg-blue-950/60 dark:text-blue-200"
                    >
                      <Icon className="h-3.5 w-3.5" />
                    </span>
                    <p className="text-xsm text-[var(--term-fg)] break-keep leading-snug">
                      {q.body}
                    </p>
                  </article>
                </li>

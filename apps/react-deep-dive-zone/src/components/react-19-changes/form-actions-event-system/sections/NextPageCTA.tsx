...
import { ArrowRightIcon, ExternalLinkIcon, RotateCcwIcon, TargetIcon, ZapIcon } from '../icons';

type Props = { content: FormActionsEventSystemContent['nextCTA'] };
...
export const NextPageCTA = ({ content }: Props) => (
  <section
    ...
    className={cn(
      ...
      'border-blue-700/60 bg-gradient-to-br from-blue-950 via-blue-900 to-cyan-900',
  ...
  >
    ...
    <div
      aria-hidden="true"
      className="absolute -right-20 -top-20 h-64 w-64 rounded-full bg-cyan-400/30 blur-3xl pointer-events-none"
    />
      ...
      <div className="flex justify-center lg:justify-start">
        <span
          ...
          className={cn(
            ...
            'border-2 border-cyan-300/60 bg-white/10 backdrop-blur-sm shadow-[0_4px_12px_rgba(34,211,238,0.3)]',
          )}
        >
          <ZapIcon className="h-8 w-8 sm:h-10 sm:w-10 text-cyan-200" />
        </span>
      </div>

      <div className="flex flex-col gap-2 min-w-0">
        <span className="text-[10px] sm:text-xsm font-mono font-bold uppercase tracking-wider text-cyan-300">
          {content.eyebrow}
        </span>
        <h2
          id="next-cta-heading"
        ...
        >
          <span className="block text-cyan-200/90 text-sm sm:text-md font-mono uppercase tracking-wider">

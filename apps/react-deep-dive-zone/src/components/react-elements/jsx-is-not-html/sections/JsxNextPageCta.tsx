...
import { NextCodeIllustration } from '../components/NextCodeIllustration';
import type { JsxIsNotHtmlContent } from '../content';
import { ArrowRightIcon, RotateIcon } from '../icons';

type Props = { content: JsxIsNotHtmlContent['next'] };

export const JsxNextPageCta = ({ content }: Props) => (
  <section aria-labelledby="heading-next" className="space-y-md">
    ...
    >
      <span
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_18%_28%,rgba(56,189,248,0.18),transparent_60%),radial-gradient(circle_at_82%_72%,rgba(167,139,250,0.18),transparent_55%)]"
      />
      <span
        aria-hidden="true"
      ...
      <div className="relative grid grid-cols-1 lg:grid-cols-[auto_minmax(0,1fr)_auto] gap-md sm:gap-lg items-center">
        ...
        <div className="flex justify-center lg:justify-start">
          <NextCodeIllustration className="h-28 w-32 sm:h-32 sm:w-36" />
        </div>

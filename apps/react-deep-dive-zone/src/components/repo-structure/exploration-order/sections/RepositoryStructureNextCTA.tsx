...
import { BookCompassIllustration } from '../components/BookCompassIllustration';
import type { ExplorationContent } from '../content';
import { ArrowRightIcon, RotateIcon } from '../icons';

type Props = { content: ExplorationContent['nextStep'] };

export const RepositoryStructureNextCTA = ({ content }: Props) => {
  return (
    <section aria-labelledby="heading-next" className="space-y-md">
      ...
      >
        <span
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_18%_30%,rgba(56,189,248,0.22),transparent_55%),radial-gradient(circle_at_85%_75%,rgba(167,139,250,0.20),transparent_55%)]"
        />

        <div className="relative grid grid-cols-1 lg:grid-cols-[auto_minmax(0,1fr)_auto] gap-md sm:gap-lg items-center">
          <div className="flex justify-center lg:justify-start">
            <BookCompassIllustration className="h-24 w-28 sm:h-28 sm:w-32" />
          </div>

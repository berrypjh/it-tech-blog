...
import { MapCompassIllustration } from '../components/MapCompassIllustration';
import type { ChangelogContent } from '../content';
import { ArrowRightIcon, RotateIcon } from '../icons';

type Props = { content: ChangelogContent['nextStep'] };

export const ChangelogReleasesNextCTA = ({ content }: Props) => {
  return (
    <section aria-labelledby="heading-next" className="space-y-md">
      ...
      >
        <span
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_18%_30%,rgba(56,189,248,0.22),transparent_55%),radial-gradient(circle_at_85%_75%,rgba(45,212,191,0.20),transparent_55%)]"
        />

        <div className="relative grid grid-cols-1 lg:grid-cols-[auto_minmax(0,1fr)_auto] gap-md sm:gap-lg items-center">
          <div className="flex justify-center lg:justify-start">
            <MapCompassIllustration className="h-24 w-28 sm:h-28 sm:w-32" />
          </div>
          ...
          <div className="flex flex-col gap-2 min-w-0 text-center lg:text-left">
            ...
            <p className="text-sm sm:text-md leading-relaxed break-keep text-slate-200">
              ...
              <span className="font-bold font-mono text-teal-300">{content.accentLine.accent}</span>
          ...
          <div className="flex flex-col gap-2 w-full lg:w-auto">
            <Link
              ...
              className={cn(
                ...
                'bg-sky-400 text-slate-950 text-xsm font-bold',
                'transition-colors hover:bg-sky-300',
                'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-200 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900',
              )}
            >
              {content.primaryCta}
              <ArrowRightIcon

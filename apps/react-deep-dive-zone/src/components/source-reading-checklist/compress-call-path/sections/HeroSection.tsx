...
type Props = { content: CallPathCompressionContent['hero'] };
...
export const HeroSection = ({ content }: Props) => {
  return (
    <section aria-labelledby="hero-heading" className="relative">
      <TerminalPrompt
        command="cat"
        path="source-reading-checklist/compress-call-path.md"
        suffix={
          <span className="text-[var(--term-dim)]">
            {' // scatter → compress → single call path'}
          </span>
        }
      />
        ...
        >
          ...
          >
            ...
            >
              ...
              {/* Scattered grid (2-col irregular) */}
              <ul className="grid grid-cols-1 gap-1.5">
                {content.leftFunctions.map((fn, i) => (
                  <li
                    key={fn}
                    className={cn(
            ...
            <div className="relative flex sm:flex-col items-center justify-center gap-2 sm:py-md">
              ...
              >
                <ArrowRightLeftIcon className="h-3 w-3" aria-hidden="true" />
                {content.connectorLabel}
              </span>
              <span className="hidden sm:block text-[10px] font-mono text-[var(--term-muted)] break-keep text-center max-w-[12ch]">
                {content.connectorSub}
              </span>
            </div>

            {/* Right mini panel: compressed call path */}
            <article
              className={cn(
                'rounded-xl border-2 p-3 sm:p-md',

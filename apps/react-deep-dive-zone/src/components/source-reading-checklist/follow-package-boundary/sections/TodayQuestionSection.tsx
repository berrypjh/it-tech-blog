...
type Props = { content: FollowPackageBoundaryContent['todayQuestion'] };

const renderLine = (
  line: string,
  emphasize: FollowPackageBoundaryContent['todayQuestion']['emphasize'],
) => {
  const parts = line.split(/(\{reactWord\}|\{reconcilerWord\})/g);
          ...
          <div className="flex flex-col gap-md">
            ...
            <ul className="flex flex-wrap gap-2">
              {content.badges.map((badge) => (
                <li key={badge.label}>
                  {badge.packageKey ? (
                    <PackageBadge packageKey={badge.packageKey} size="md" strong>
                      {badge.label}
                    </PackageBadge>
                  ) : (
                    <span

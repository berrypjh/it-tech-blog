...
import { ArrowRightIcon, FileCodeIcon, MapIcon, PackageIcon, RepeatIcon } from '../icons';
...
export const HeroSection = ({ content }: Props) => {
  return (
    <section aria-labelledby="hero-heading" className="relative">
      <TerminalPrompt
        command="cat"
        path="source-reading-checklist/follow-package-boundary.md"
        suffix={
          <span className="text-[var(--term-dim)]">
            {' // a file is a dot — a package is a map'}
          </span>
        }
      />
      ...
      <div className="mt-md grid grid-cols-1 gap-lg lg:gap-xl lg:grid-cols-[minmax(0,_40fr)_minmax(0,_60fr)] items-stretch">
        ...
        {/* RIGHT — Visual: file-first vs package map */}
        <div
          className={cn(
            ...
            >
              ...
              <ul className="flex flex-col gap-1.5">
                {content.rightMap.map((pkg) => (
                  ...
                  >
                    <PackageBadge packageKey={pkg.name} size="sm">
                      {pkg.name}
                    </PackageBadge>
                    <span
                      aria-hidden="true"
                      className="text-[10px] text-[var(--term-dim)] shrink-0"
                    >
                      →
                    </span>
                    <span className="text-[11px] text-[var(--term-muted)] break-keep truncate">
                      {pkg.role}
                    </span>
                  </li>
                ))}

...
type Props = { content: ReactElementObjectStructureContent['checkpoint'] };

export const ElementSourceCheckpoint = ({ content }: Props) => (
  <section id="checkpoint" aria-labelledby="heading-checkpoint" className="space-y-md scroll-mt-xl">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,_0.7fr)_minmax(0,_1.3fr)] gap-md items-stretch">
      ...
      >
        ...
        <InfoBlock
          icon={<CodeIcon className="h-4 w-4" aria-hidden="true" />}
          label={content.functionLabel}
          value={content.functionName}
          mono
          highlight
        />
        ...
        <div className="flex flex-col gap-sm">
          <div className="flex items-center gap-sm">
            <span
              aria-hidden="true"
              className="inline-flex items-center justify-center w-9 h-9 rounded-lg border border-amber-300/80 bg-amber-100 text-amber-700 dark:border-amber-800/70 dark:bg-amber-950/60 dark:text-amber-200"
            >
...
const InfoBlock = ({
  ...
  <div className="flex items-start gap-sm">
    ...
    <div className="flex flex-col gap-1 min-w-0">
      ...
      {highlight ? (
        <span className="inline-flex w-fit items-center rounded-full border border-sky-300/80 bg-sky-50 px-2 py-0.5 text-xsm font-mono font-bold text-sky-700 dark:border-sky-800/70 dark:bg-sky-950/60 dark:text-sky-200">
          {value}
        </span>
      ) : mono ? (

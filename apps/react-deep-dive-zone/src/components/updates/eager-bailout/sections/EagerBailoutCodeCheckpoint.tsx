...
import { CircleHelpIcon, FileCodeIcon, FileTextIcon, SearchIcon, SparklesIcon } from '../icons';
...
const BOOLEAN_TOKENS = new Set(['true', 'false', 'null']);
...
export const EagerBailoutCodeCheckpoint = ({ content }: Props) => (
  <section id="checkpoint" aria-labelledby="heading-checkpoint" className="space-y-md scroll-mt-xl">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,_0.82fr)_minmax(0,_1.45fr)] gap-md lg:gap-lg items-stretch">
      {/* Info card */}
      <article
        className={cn(
      ...
      >
        ...
        <InfoRow
          ...
          label={content.info.focusLabel}
          value={content.info.focusText}
          accent
        />
        <InfoRow
      ...
      <div className="min-w-0">
        <LightCodePanel
          ...
          highlightFromLine={content.code.highlightFromLine}
          highlightToLine={content.code.highlightToLine}
...
type LightCodePanelProps = {
  ...
  callout: EagerBailoutContent['checkpoint']['callout'];
};
    ...
    >
      ...
      <pre className="overflow-x-auto px-md py-md font-mono leading-[1.85] text-[12.5px] sm:text-[13px]">
        <code>
          {lines.map((line, i) => {
            const lineNo = i + 1;
            const highlighted = lineNo >= highlightFromLine && lineNo <= highlightToLine;
            return (
              <div
                key={i}
                className={cn(
      ...
      <div
        className={cn(
          'flex items-start gap-sm border-t px-md py-3',
          'border-[var(--term-border)] bg-emerald-50/40',
          'dark:bg-emerald-950/20 dark:border-emerald-800/40',
        )}
      >
        <span
          aria-hidden="true"
        ...
        <div className="flex flex-col gap-0.5">
          <span className="text-xsm font-bold text-emerald-900 dark:text-emerald-100 break-keep">
            {callout.line1}
          </span>
          <span className="text-[10px] font-mono text-emerald-800/80 dark:text-emerald-200/80">
            {callout.line2}
          </span>
        </div>
      </div>

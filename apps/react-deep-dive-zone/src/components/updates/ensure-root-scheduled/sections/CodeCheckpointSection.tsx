...
type Props = { content: EnsureRootScheduledContent['checkpoint'] };

const HIGHLIGHT_TOKENS = new Set(['ensureRootIsScheduled', 'root', 'FiberRoot']);

const KEYWORDS = new Set(['export', 'function']);

const COMMENT_HIGHLIGHTS = [
  { match: /root schedule/i, color: 'text-emerald-700 dark:text-emerald-300 font-bold' },
  { match: /pending microtask/i, color: 'text-violet-700 dark:text-violet-300 font-bold' },
];
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,_0.78fr)_minmax(0,_1.45fr)] gap-md lg:gap-lg items-stretch">
      ...
      <div className="min-w-0 grid grid-cols-1 xl:grid-cols-[minmax(0,_1fr)_minmax(0,_0.5fr)] gap-md items-start">
        <LightCodePanel
          ...
          highlightLines={content.callouts.map((c) => c.linkedLine)}
        />
...
const calloutToneClass = {
  mint: {
    ...
    numberBox: 'bg-emerald-600 text-white dark:bg-emerald-500 dark:text-slate-950',
    heading: 'text-emerald-700/80 dark:text-emerald-300/80',
    title: 'text-emerald-900 dark:text-emerald-100',
  },
  violet: {
    border: 'border-violet-300/80 dark:border-violet-700/70',
    bg: 'bg-violet-50/70 dark:bg-violet-950/30',
    numberBox: 'bg-violet-600 text-white dark:bg-violet-500 dark:text-slate-950',
    heading: 'text-violet-700/80 dark:text-violet-300/80',
    title: 'text-violet-900 dark:text-violet-100',
  },
} as const;

type CalloutCardProps = {
  callout: EnsureRootScheduledContent['checkpoint']['callouts'][number];
};

const CalloutCard = ({ callout }: CalloutCardProps) => {
  const t = calloutToneClass[callout.tone];
  return (
    <article
      className={cn(
    ...
    >
      ...
      <div className="flex flex-col gap-1 min-w-0">
        <span className={cn('text-[10px] font-mono uppercase tracking-wider font-bold', t.heading)}>
          {callout.heading}
        </span>
        <h4 className={cn('text-xsm sm:text-sm font-bold leading-tight break-keep', t.title)}>
          {callout.title}
        </h4>
        <span
          aria-hidden="true"
...
const LightCodePanel = ({ fileName, rightLabel, content, highlightLines }: LightCodePanelProps) => {
            ...
            return (
              <div
                ...
                className={cn(
                  'flex',
                  highlighted &&
                    'rounded-md border-2 border-sky-400/80 bg-sky-50/80 dark:border-sky-600/70 dark:bg-sky-950/40',
                )}
              >
                <span
                  aria-hidden="true"
...
const highlightLine = (line: string): React.ReactNode => {
  ...
  if (commentMatch) {
    ...
    // Apply phrase highlighting inside comment
    for (const ch of COMMENT_HIGHLIGHTS) {
      const m = comment.match(ch.match);
      if (m) {
        const idx = comment.indexOf(m[0]);
        const before = comment.slice(0, idx);
        const hit = comment.slice(idx, idx + m[0].length);
        const after = comment.slice(idx + m[0].length);
        return (
          <>
            <span>{indent}</span>
            <span className="text-slate-500 italic dark:text-slate-500">{before}</span>
            <span className={cn('italic', ch.color)}>{hit}</span>
            <span className="text-slate-500 italic dark:text-slate-500">{after}</span>
          </>
        );
      }
    }
    return (
      <>
        <span>{indent}</span>
        <span className="text-slate-500 italic dark:text-slate-500">{comment}</span>
      </>
    );
  }
  ...
  return tokens.map((tok, idx) => {
    ...
    if (HIGHLIGHT_TOKENS.has(tok)) {
      return (
        <span key={idx} className="text-sky-700 font-bold dark:text-amber-300">
          {tok}
        </span>
      );
    }

...
type Props = { content: RnContent['concept'] };

export const ConceptSection = ({ content }: Props) => {
  return (
    <section aria-labelledby="heading-concept" className="space-y-md">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-md items-stretch">
        ...
        >
          <header className="flex items-center gap-sm">
            <span
              aria-hidden="true"
              className={cn(
                'inline-flex items-center justify-center w-12 h-12 rounded-full text-base font-bold font-mono',
        ...
        <article
          className={cn(
            'relative flex h-full flex-col gap-md rounded-2xl border p-md sm:p-lg overflow-hidden',
            'border-teal-300/80 bg-teal-50/70 dark:border-teal-800/70 dark:bg-teal-950/30',
            'shadow-[0_2px_0_var(--term-border)]',
          )}
        >
          <span

...
const RECONCILER_HIGHLIGHTS = new Set(['createFiberFromElement', 'createFiberFromTypeAndProps']);

/**
 * react-reconciler 페이지용 dark navy 코드 패널.
 * teal accent + 우상단 JS 배지 + 하단 footer 버튼.
 */
export const ReactFiberCodePanel = ({
  caption,
  code,
  primaryLabel,
  secondaryLabel,
    ...
    >
      <div className="flex items-center justify-between gap-sm px-md py-2 border-b border-slate-800 bg-slate-900">
        ...
        <div className="flex items-center gap-2">
          ...
          <span
            className={cn(
              'inline-flex items-center rounded-md border border-teal-500/60 bg-teal-500/15 px-1.5 py-0.5',
              'text-[10px] font-mono font-bold uppercase tracking-wider text-teal-200',
            )}
          >
            js
          </span>
...
const highlight = (line: string): React.ReactNode => {
  ...
  return tokens.map((tok, idx) => {
    ...
    if (RECONCILER_HIGHLIGHTS.has(tok)) {
      return (

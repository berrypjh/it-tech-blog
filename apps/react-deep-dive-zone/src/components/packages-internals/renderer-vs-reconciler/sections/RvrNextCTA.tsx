...
import { SchedulerClockIllustration } from '../components/SchedulerClockIllustration';
import type { RvrContent } from '../content';
import { ArrowRightIcon, RotateIcon } from '../icons';

type Props = { content: RvrContent['nextStep'] };

export const RvrNextCTA = ({ content }: Props) => {
  return (
    <section aria-labelledby="heading-next" className="space-y-md">
      ...
      >
        <span
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_18%_28%,rgba(56,189,248,0.22),transparent_60%),radial-gradient(circle_at_82%_72%,rgba(45,212,191,0.18),transparent_55%)]"
        />
        <span
          aria-hidden="true"
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-[auto_minmax(0,1fr)_auto] gap-md sm:gap-lg items-center">
          <div className="flex justify-center lg:justify-start">
            <SchedulerClockIllustration className="h-28 w-32 sm:h-32 sm:w-36" />
          </div>
          ...
          <div className="flex flex-col gap-2 min-w-0 text-center lg:text-left">
            ...
            <p className="text-sm sm:text-md leading-relaxed break-keep text-slate-200">
              ...
              <span className="font-bold font-mono text-sky-300">{content.line2Accent}</span>

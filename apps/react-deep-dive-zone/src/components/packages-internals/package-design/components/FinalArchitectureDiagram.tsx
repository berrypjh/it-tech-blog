 ...
 */
export const FinalArchitectureDiagram = ({ main, scheduler, shared, a11y }: Props) => {
  return (
    <div
      className={cx(
        ...
        <ol className="flex flex-col items-center gap-sm" aria-hidden="true">
          {main.map((node, i) => (
            <li key={node.id} className="flex flex-col items-center gap-sm w-full">
              <FlowBox node={node} emphasized={i === 2 /* reconciler */} />
...
const FlowBox = ({ node, emphasized }: FlowBoxProps) => {
  ...
  return (
    ...
    >
      ...
      {node.description && <p className={cx(DESCRIPTION_CLASS, 'mt-1')}>{node.description}</p>}
    </article>
  );
};

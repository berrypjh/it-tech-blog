...
import type { QuizCard, ReactPackageContent } from '../content';
import { CheckCircleIcon, HelpCircleIcon } from '../icons';

type Props = { content: ReactPackageContent['quiz'] };
...
const QuizCardView = ({ card, index }: { card: QuizCard; index: number }) => {
  return (
    ...
    >
      ...
      >
        <span className="text-[10px] uppercase tracking-wider text-sky-700 dark:text-sky-300 font-bold font-mono pt-0.5">
          A.
        </span>
        <span className="text-sm font-bold font-mono break-words text-sky-800 dark:text-sky-100">
          {card.answer}
        </span>
      </div>

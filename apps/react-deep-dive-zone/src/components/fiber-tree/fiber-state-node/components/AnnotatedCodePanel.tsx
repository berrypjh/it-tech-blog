...
const lineHighlight: Record<ToneKey, string> = {
  sky: 'bg-sky-500/15',
  cyan: 'bg-cyan-500/15',
...
export const AnnotatedCodePanel = ({ code, fileName, language, annotations }: Props) => {
        ...
        {lines.map((line, i) => {
          ...
          return (
            <div
              key={i}
              className={cn(
                'grid grid-cols-[1fr_minmax(120px,_160px)] sm:grid-cols-[1fr_minmax(140px,_180px)] items-stretch',
                ann && lineHighlight[ann.annotation.tone],
              )}
            >
              <div className="flex px-md font-mono leading-[1.75] text-[12.5px]">
                <span
                  aria-hidden="true"
                  ...
                  <div className="absolute inset-0 flex items-center gap-1.5 px-2">
                    <span
                      ...
                      className={cn(
                        ...
                        bracketColor[ann.annotation.tone],
                        ann.position === 'top' && 'border-t-2 rounded-tr-md',
                        ann.position === 'bottom' && 'border-b-2 rounded-br-md',
                        ann.position === 'single' && 'border-t-2 border-b-2 rounded-r-md',
                      )}
                    />
                    {(ann.position === 'middle' || ann.position === 'single') && (
                      <span
                        className={cn(
                          'inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-bold tracking-tight whitespace-nowrap',
                          labelStyle[ann.annotation.tone],
                        )}
                      >

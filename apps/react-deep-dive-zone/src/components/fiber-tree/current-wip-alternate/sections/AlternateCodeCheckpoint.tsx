...
export const AlternateCodeCheckpoint = ({ content }: Props) => (
  <section id="checkpoint" aria-labelledby="heading-checkpoint" className="space-y-md scroll-mt-xl">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,_0.4fr)_minmax(0,_1fr)] gap-md lg:gap-lg items-start">
      <article
        className={cn(
      ...
      >
        ...
        <dl className="mt-sm flex flex-col gap-sm">
          <InfoRow icon={<FileCodeIcon className="h-4 w-4" />} label={content.info.fileLabel}>
            <span className="font-mono text-xsm text-[var(--term-fg)] break-all">
              {content.info.file}
            </span>
          </InfoRow>
          <InfoRow icon={<BracesIcon className="h-4 w-4" />} label={content.info.functionLabel}>
            <code className="font-mono text-xsm font-bold text-violet-700 dark:text-violet-300 break-all">
              {content.info.functionName}
            </code>
          </InfoRow>

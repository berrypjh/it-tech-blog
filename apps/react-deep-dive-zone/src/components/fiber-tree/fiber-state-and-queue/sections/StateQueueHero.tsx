...
export const StateQueueHero = ({ content }: Props) => (
            ...
            {content.title.line2.split(content.emphasis).map((part, i, arr) => (
              <span key={i}>
                ...
                {i < arr.length - 1 && (
                  <span
                    className={cn(
                      'bg-gradient-to-r from-emerald-600 via-sky-500 to-violet-500 bg-clip-text text-transparent',
                      'dark:from-emerald-300 dark:via-sky-300 dark:to-violet-300',
                    )}
                  >
                    {content.emphasis}
                  </span>

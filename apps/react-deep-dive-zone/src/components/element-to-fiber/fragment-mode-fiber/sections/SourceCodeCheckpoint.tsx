...
type Props = { content: FragmentModeFiberContent['checkpoint'] };

const emphasisRowTone = {
  fragment: 'bg-violet-500/10 border-l-2 border-violet-400/80',
  mode: 'bg-emerald-500/10 border-l-2 border-emerald-400/80',
} as const;

const emphasisLineNumberTone = {
  fragment: 'text-violet-300',
  mode: 'text-emerald-300',
} as const;

const emphasisTextTone = {
  fragment: 'text-violet-200',
  mode: 'text-emerald-200',
} as const;
  ...
  <section id="checkpoint" aria-labelledby="heading-checkpoint" className="space-y-md scroll-mt-xl">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,_0.78fr)_minmax(0,_1.45fr)] gap-md items-stretch">
      ...
      >
        ...
        <InfoItem icon={<TargetIcon className="h-4 w-4" />} label={content.spotLabel}>
          <code
            className={cn(
...
const HighlightedCodePanel = ({
  ...
  caption: string;
  lines: { line: string; emphasis?: 'fragment' | 'mode' }[];
}) => (
  <div
    className={cn(

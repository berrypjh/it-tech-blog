...
import { ArrowRightIcon, RefreshIcon, RocketIcon, TrophyIcon } from '../icons';

type Props = { content: FiberWhyNeededContent['next'] };

export const FinishNextStepCTA = ({ content }: Props) => (
  <section id="next" aria-labelledby="heading-next" className="space-y-md scroll-mt-xl">
    ...
    >
      <span
        aria-hidden="true"
        className="pointer-events-none absolute -top-6 -right-6 w-32 h-32 rounded-full bg-sky-300/20 blur-2xl"
      />
      ...
      <div className="relative grid grid-cols-1 lg:grid-cols-[auto_minmax(0,1fr)_auto] gap-md sm:gap-lg items-center">
        ...
        <p className="text-sm sm:text-md leading-relaxed text-[var(--term-fg)] break-keep max-w-[62ch] text-center lg:text-left">
          {content.description}
        </p>

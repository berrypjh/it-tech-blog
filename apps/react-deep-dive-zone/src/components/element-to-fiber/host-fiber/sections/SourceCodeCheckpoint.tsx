...
type Props = { content: HostComponentFiberContent['checkpoint'] };
...
export const SourceCodeCheckpoint = ({ content }: Props) => (
  <section id="checkpoint" aria-labelledby="heading-checkpoint" className="space-y-md scroll-mt-xl">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,_0.78fr)_minmax(0,_1.45fr)] gap-md items-stretch">
      ...
      {/* Right: code panel with highlight */}
      ...
      <code className="text-slate-100">
        {lines.map((row, i) => (
          <div
            ...
            className={cn(
              'flex',
              row.emphasis && 'bg-emerald-500/10 -mx-md px-md border-l-2 border-emerald-400/80',
            )}
          >
            <span
              aria-hidden="true"
              className={cn(
                ...
                row.emphasis ? 'text-emerald-300' : 'text-slate-600',
              )}
            >
              {i + 1}
            </span>
            <span className={cn('whitespace-pre', row.emphasis && 'text-emerald-200 font-bold')}>
              {row.line}
            </span>
          </div>
        ))}

...
const emphasisRowTone = {
  check: 'bg-sky-500/15 border-l-2 border-sky-400/80',
  wireA: 'bg-violet-500/15 border-l-2 border-violet-400/80',
  wireB: 'bg-violet-500/20 border-l-2 border-violet-400/80',
  reuse: 'bg-amber-500/15 border-l-2 border-amber-400/80',
} as const;

const emphasisLineNumberTone = {
  check: 'text-sky-300',
  wireA: 'text-violet-300',
  wireB: 'text-violet-300',
  reuse: 'text-amber-300',
} as const;

const emphasisTextTone = {
  check: 'text-sky-200',
  wireA: 'text-violet-200',
  wireB: 'text-violet-100',
  reuse: 'text-amber-200',
} as const;
          ...
          {content.annotations.map((a) => (
            <li key={a.id}>
              ...
              >
                <span
                  aria-hidden="true"
                  className={cn('inline-block w-1 h-3 rounded-sm', annotationDot[a.tone])}
                />
                {a.label}
              </span>
            </li>
...
const HighlightedCodePanel = ({
  ...
  caption: string;
  lines: { line: string; emphasis?: 'check' | 'wireA' | 'wireB' | 'reuse' }[];
}) => (
  <div
    className={cn(
    ...
    <pre className="overflow-x-auto px-md py-md font-mono leading-[1.7] text-[13px] sm:text-[14px]">
      <code className="text-slate-100">
        {lines.map((row, i) => (
          <div
            key={i}
            className={cn('flex', row.emphasis && `-mx-md px-md ${emphasisRowTone[row.emphasis]}`)}
          >
            <span
              aria-hidden="true"
              className={cn(
                'select-none w-7 shrink-0 pr-3 text-right tabular-nums',
                row.emphasis ? emphasisLineNumberTone[row.emphasis] : 'text-slate-600',
              )}
            >
              {i + 1}
            </span>
            <span
              className={cn(
                'whitespace-pre',
                row.emphasis && `${emphasisTextTone[row.emphasis]} font-bold`,
              )}
            >
              {row.line || ' '}
            </span>

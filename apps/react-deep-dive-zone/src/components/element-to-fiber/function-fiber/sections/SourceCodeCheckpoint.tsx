...
type Props = { content: FunctionClassComponentFiberContent['checkpoint'] };

const emphasisRowTone = {
  class: 'bg-violet-500/10 border-l-2 border-violet-400/80',
  function: 'bg-emerald-500/10 border-l-2 border-emerald-400/80',
  string: 'bg-sky-500/10 border-l-2 border-sky-400/80',
} as const;
  ...
  <section id="checkpoint" aria-labelledby="heading-checkpoint" className="space-y-md scroll-mt-xl">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,_0.78fr)_minmax(0,_1.45fr)] gap-md items-stretch">
      ...
      >
        ...
        <InfoItem icon={<Settings2Icon className="h-4 w-4" />} label={content.spotLabel}>
          <code
            className={cn(
      ...
      <div className="flex flex-col gap-sm min-w-0">
        ...
        <ul className="flex flex-wrap gap-2" aria-label="코드 분기 영역 라벨">
...
const HighlightedCodePanel = ({
  ...
  caption: string;
  lines: { line: string; emphasis?: 'class' | 'function' | 'string' }[];
}) => (
  <div
    className={cn(

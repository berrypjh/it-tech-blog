...
type Props = { content: RootPendingWorkContent['code'] };

const KEYWORDS = new Set(['function', 'const', 'return']);
const FN_NAMES = new Set(['markRootUpdated', 'ensureRootIsScheduled']);
  ...
  if (FN_NAMES.has(tok))
    return (
      <span
        key={i}
        className={cn(
          tok === 'markRootUpdated' && 'text-cyan-300 font-semibold',
          tok === 'ensureRootIsScheduled' && 'text-amber-200 font-semibold',
        )}
      >
        {tok}
      </span>
    );
  if (tok === 'root' || tok === 'lane' || tok === 'updateLane')
    return (
...
const CodeCard = ({ card }: { card: { title: string; fileLabel: string; code: string } }) => {
...
export const RootPendingWorkCodePreview = ({ content }: Props) => (
  <section aria-labelledby="heading-code">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-md items-stretch">
      <CodeCard card={content.cardA} />
      <CodeCard card={content.cardB} />
      ...
      >
        ...
        <ul className="flex flex-wrap gap-2">
          {['markRootUpdated', 'ensureRootIsScheduled', 'root.pendingLanes'].map((api) => (
            <li key={api}>
              <code
                className={cn(

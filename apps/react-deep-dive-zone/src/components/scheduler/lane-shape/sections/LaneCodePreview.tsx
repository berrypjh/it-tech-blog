...
type Props = { content: LaneBitmaskContent['code'] };

const KEYWORDS = new Set(['export', 'const']);
const NUMBER_RE = /^(0b[01]+|\d+)$/;
const IDENTS = new Set(['TotalLanes', 'NoLanes', 'SyncLane', 'InputContinuousLane']);
...
const renderToken = (tok: string, i: number) => {
  ...
  if (IDENTS.has(tok))
    return (
      <span key={i} className="text-cyan-200 font-semibold">
        {tok}
      </span>
    );
...
export const LaneCodePreview = ({ content }: Props) => {

...
type Props = { content: RenderYieldingContent['code'] };

const KEYWORDS = new Set(['const', 'if', 'return', 'break']);
const FN_NAMES = new Set(['shouldYieldToHost', 'getCurrentTime']);
...
const renderToken = (tok: string, i: number) => {
  ...
  if (KEYWORDS.has(tok))
    return (
      <span
        key={i}
        className={cn('font-semibold', tok === 'break' ? 'text-violet-300' : 'text-pink-300')}
      >
        {tok}
      </span>
    );
  ...
  if (tok === 'expirationTime' || tok === 'frameInterval')
    return (
  ...
  if (LITERALS.has(tok))
    return (
      <span key={i} className={cn(tok === 'true' ? 'text-emerald-300' : 'text-rose-300')}>
        {tok}
      </span>
    );
  return (
...
export const RenderYieldingCodePreview = ({ content }: Props) => (
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-md items-stretch">
      ...
      <article
        className={cn(
          ...
          'border-blue-200/80 bg-gradient-to-br from-blue-50/70 via-white to-emerald-50/40',
          'dark:border-blue-800/60 dark:from-blue-950/30 dark:via-[var(--term-bg)] dark:to-emerald-950/20',
      ...
      >
        ...
        <ul className="flex flex-wrap gap-2">
          {['workLoop', 'shouldYieldToHost', 'frameInterval', 'continuation'].map((api) => (
            <li key={api}>
              <code
                className={cn(

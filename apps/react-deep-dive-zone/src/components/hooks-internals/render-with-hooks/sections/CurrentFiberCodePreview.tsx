...
const KEYWORDS = new Set(['const', 'let', 'var', 'function', 'return', 'export']);
const NULLISH = new Set(['null', 'undefined', 'true', 'false']);
...
const renderToken = (tok: string, i: number) => {
  ...
  if (tok === 'currentlyRenderingFiber' || tok === 'workInProgress')
    return (
      <span key={i} className="text-violet-300">
        {tok}
      </span>
    );
  if (tok === 'memoizedState' || tok === 'updateQueue' || tok === 'lanes')
    return (
...
export const CurrentFiberCodePreview = ({ content }: Props) => {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] gap-md">
        {/* Code panel */}
        ...
        >
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-[var(--term-border)] bg-[var(--term-border)]/20 px-2.5 py-0.5 text-[10px] font-mono uppercase tracking-wider text-[var(--term-muted)]">
              {content.fileLabel}
            </span>
          ...
          <ul className="flex flex-col gap-1.5 mt-auto">
            <li className="flex items-start gap-2 text-[11px] sm:text-xsm text-[var(--term-fg)]">
              ...
              <span className="break-keep">
                <code className="font-mono">currentlyRenderingFiber</code>가 가리키는 Fiber에 상태가
                붙는다
              </span>
            </li>
            <li className="flex items-start gap-2 text-[11px] sm:text-xsm text-[var(--term-fg)]">
              ...
              />
              <span className="break-keep">
                <code className="font-mono">memoizedState</code>는 첫 Hook 노드를 받기 위해{' '}
                <code className="font-mono">null</code>로 초기화
              </span>
            </li>
          </ul>
          ...
          <a
            ...
            className={cn(
              'group inline-flex items-center justify-center gap-2 rounded-xl border px-4 py-2.5',
              'border-blue-300/70 bg-blue-50 text-blue-700 font-bold text-xsm sm:text-sm',

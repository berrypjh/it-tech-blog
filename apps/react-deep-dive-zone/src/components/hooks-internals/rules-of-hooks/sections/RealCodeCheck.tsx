...
type Props = { content: RulesOfHooksContent['realCode'] };

const KEYWORDS = new Set(['function', 'if', 'return']);
...
const renderToken = (tok: string, i: number) => {
  ...
  if (tok === 'updateHookTypesDev' || tok === 'warnOnHookMismatchInDev')
    return (
  ...
  if (
    tok === '__DEV__' ||
    tok === 'hookTypesDev' ||
    tok === 'hookTypesUpdateIndexDev' ||
    tok === 'currentHookNameInDev'
  )
    return (
...
export const RealCodeCheck = ({ content }: Props) => {
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-950 shadow-[0_2px_0_var(--term-border)]">
          <div className="flex items-center justify-between gap-2 border-b border-slate-800 px-md py-2">
            <div className="flex items-center gap-1.5">
              ...
              />
              <span className="ml-2 text-[10px] font-mono text-slate-500">updateHookTypesDev</span>
            </div>
        ...
        >
          ...
          <ul className="flex flex-col gap-1.5">
            <li className="flex items-start gap-2 text-[11px] sm:text-xsm text-[var(--term-fg)]">
              <span
                aria-hidden="true"
                className="mt-1.5 inline-block h-1.5 w-1.5 rounded-full bg-pink-500 dark:bg-pink-400"
              />
              <span className="break-keep">
                <code className="font-mono break-all">hookTypesDev</code>에 호출 순서대로 Hook 이름
                저장
              </span>
            </li>
            <li className="flex items-start gap-2 text-[11px] sm:text-xsm text-[var(--term-fg)]">
              ...
              />
              <span className="break-keep">
                <code className="font-mono break-all">currentHookNameInDev</code>와 비교해 불일치
                감지
              </span>
            </li>
            <li className="flex items-start gap-2 text-[11px] sm:text-xsm text-[var(--term-fg)]">
              <span
                aria-hidden="true"
                className="mt-1.5 inline-block h-1.5 w-1.5 rounded-full bg-rose-500 dark:bg-rose-400"
              />
              <span className="break-keep">
                불일치 시 <code className="font-mono break-all">warnOnHookMismatchInDev</code> 호출
              </span>
            </li>
          </ul>

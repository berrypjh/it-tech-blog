...
type Props = { content: SetStateFlowContent['realCode'] };

const KEYWORDS = new Set(['const', 'let', 'var', 'function', 'return']);
const NULLISH = new Set(['null', 'true', 'false']);
...
const renderToken = (tok: string, i: number) => {
  ...
  if (tok === 'requestUpdateLane')
    return (
      <span key={i} className="text-teal-300">
        {tok}
      </span>
    );
  if (tok === 'update' || tok === 'fiber')
    return (
...
export const RealUpdateCodePreview = ({ content }: Props) => {

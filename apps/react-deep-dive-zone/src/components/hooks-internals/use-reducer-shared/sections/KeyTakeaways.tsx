...
import { Link2Icon, RepeatIcon, SproutIcon, TargetIcon, TrophyIcon } from '../icons';

type Props = { content: UseReducerSharedContent['summary'] };
  ...
  >
    ...
    <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-md">
      {content.items.map((item) => {
        const Icon = visualMap[item.visual];
        return (
          <li key={item.number}>
            <article
              className={cn(

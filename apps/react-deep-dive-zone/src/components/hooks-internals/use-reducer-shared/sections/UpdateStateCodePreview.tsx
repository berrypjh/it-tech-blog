...
type Props = { content: UseReducerSharedContent['updateStatePreview'] };
...
const renderToken = (tok: string, i: number) => {
  ...
  if (tok === 'updateState' || tok === 'updateReducer' || tok === 'basicStateReducer')
    return (
...
export const UpdateStateCodePreview = ({ content }: Props) => {
  const lines = content.code.split('\n');
  return (
    <section
      aria-labelledby="heading-update-state"
      className={cn(
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] gap-md lg:gap-lg">
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-950 shadow-[0_2px_0_var(--term-border)]">
          <div className="flex items-center justify-between gap-2 border-b border-slate-800 px-md py-2">
            <div className="flex items-center gap-1.5">
              ...
              />
              <span className="ml-2 text-[10px] font-mono text-slate-500">updateState</span>
            </div>
        ...
        >
          ...
          <ul className="flex flex-col gap-1.5">
            <li className="flex items-start gap-2 text-[11px] sm:text-xsm text-[var(--term-fg)]">
              ...
              <span className="break-keep">
                <code className="font-mono">updateState</code> →{' '}
                <code className="font-mono">updateReducer</code> 위임
              </span>
            </li>
            <li className="flex items-start gap-2 text-[11px] sm:text-xsm text-[var(--term-fg)]">
              <span
              ...
              />
              <span className="break-keep">
                <code className="font-mono">basicStateReducer</code>를 인자로 전달
              </span>
            </li>
          </ul>
          ...
          <a
            ...
            className={cn(
              'mt-auto group inline-flex items-center justify-center gap-2 rounded-xl border-2 px-4 py-2.5',

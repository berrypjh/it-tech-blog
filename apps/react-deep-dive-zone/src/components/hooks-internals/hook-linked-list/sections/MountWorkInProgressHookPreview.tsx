...
type Props = { content: HookLinkedListContent['mountPreview'] };

const KEYWORDS = new Set(['const', 'let', 'var', 'if', 'else', 'return']);
const NULLISH = new Set(['null', 'true', 'false', 'undefined']);
...
const renderToken = (tok: string, i: number) => {
  ...
  if (tok === 'hook' || tok === 'workInProgressHook' || tok === 'currentlyRenderingFiber')
    return (
...
export const MountWorkInProgressHookPreview = ({ content }: Props) => {
          ...
          <ul className="flex flex-col gap-1.5">
            <li className="flex items-start gap-2 text-[11px] sm:text-xsm text-[var(--term-fg)]">
              ...
              <span className="break-keep">
                첫 번째:{' '}
                <code className="font-mono">currentlyRenderingFiber.memoizedState = hook</code>
              </span>
            </li>
            <li className="flex items-start gap-2 text-[11px] sm:text-xsm text-[var(--term-fg)]">
              <span
              ...
              />
              <span className="break-keep">
                이후: <code className="font-mono">workInProgressHook.next = hook</code>
              </span>
            </li>
          </ul>

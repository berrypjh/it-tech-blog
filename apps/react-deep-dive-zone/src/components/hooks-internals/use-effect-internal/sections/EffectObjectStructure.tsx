...
const KEYWORDS = new Set(['type', 'void', 'any', 'null']);
const FIELD_NAMES = new Set(['tag', 'create', 'inst', 'destroy', 'resource', 'deps', 'next']);
...
const renderToken = (tok: string, i: number) => {
  ...
  if (tok === 'Effect' || tok === 'HookFlags' || tok === 'Array' || tok === 'mixed')
    return (
...
export const EffectObjectStructure = ({ content }: Props) => {
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-950 shadow-[0_2px_0_var(--term-border)]">
          <div className="flex items-center justify-between gap-2 border-b border-slate-800 px-md py-2">
            <div className="flex items-center gap-1.5">
              ...
              />
              <span className="ml-2 text-[10px] font-mono text-slate-500">Effect.d.ts</span>
            </div>
          ...
          {content.fields.map((field) => (
            <li key={field.name}>
              ...
              >
                <span
                  className={cx(
                    'inline-flex flex-col items-start shrink-0 rounded-lg border px-2.5 py-1.5 break-all min-w-[80px]',
                    toneChip[field.tone],
                  )}
                >
                  <code className="font-mono text-[11px] font-bold">{field.name}</code>
                  <code className={cx('font-mono text-[10px] mt-0.5', toneCommentText[field.tone])}>
                    {field.comment}
                  </code>
                </span>

...
import { BoxesIcon, LayersIcon, Link2Icon, TrophyIcon } from '../icons';

type Props = { content: UseStateInternalsContent['summary'] };

const cardIcons = [LayersIcon, BoxesIcon, Link2Icon];
  ...
  >
    ...
    <ul className="grid grid-cols-1 md:grid-cols-3 gap-md">
      {content.items.map((item, i) => {
        const Icon = cardIcons[i] ?? LayersIcon;
        return (
          <li key={item.number}>
            <article
              className={cn(

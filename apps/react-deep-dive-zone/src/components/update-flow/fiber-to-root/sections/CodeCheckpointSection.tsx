...
type Props = { content: FiberToRootContent['checkpoint'] };
...
const KEYWORDS = new Set(['export', 'function', 'let', 'while', 'return']);

const BOOLEAN_TOKENS = new Set(['null', 'true', 'false']);
...
export const CodeCheckpointSection = ({ content }: Props) => (
  <section id="checkpoint" aria-labelledby="heading-checkpoint" className="space-y-md scroll-mt-xl">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,_0.78fr)_minmax(0,_1.45fr)] gap-md lg:gap-lg items-stretch">
      ...
      <div className="min-w-0 grid grid-cols-1 xl:grid-cols-[minmax(0,_1fr)_minmax(0,_0.45fr)] gap-md items-start">
        <LightCodePanel
          fileName={content.code.fileName}
          rightLabel={content.code.rightLabel}
          content={content.code.content}
          note={content.code.note}
...
const calloutToneClass = {
  violet: {
    ...
    body: 'text-violet-900/85 dark:text-violet-100/85',
  },
  sky: {
    border: 'border-sky-300/80 dark:border-sky-700/70',
    bg: 'bg-sky-50/70 dark:bg-sky-950/30',
    numberBox: 'bg-sky-600 text-white dark:bg-sky-500 dark:text-slate-950',
...
type CalloutCardProps = {
  callout: FiberToRootContent['checkpoint']['callouts'][number];
};
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-1 min-w-0">
        ...
        <p className={cn('text-xxsm leading-snug break-keep', t.body)}>{callout.body}</p>
        <span
          aria-hidden="true"
...
const LightCodePanel = ({
  ...
  return (
    ...
    >
            ...
            return (
              <div
                ...
                className={cn(
                  'flex',
                  highlighted &&
                    'rounded-md border border-emerald-300/80 bg-emerald-50/70 dark:border-emerald-700/70 dark:bg-emerald-950/30',
                )}
              >
                <span
                  aria-hidden="true"
      ...
      <div
        className={cn(
          'flex items-center justify-end gap-2 border-t px-md py-2',
          'border-[var(--term-border)] bg-slate-50/40',
...
const highlightLine = (line: string): React.ReactNode => {
  ...
  // Split into tokens including dot separated identifiers as combined
  const rawTokens = line.split(/(\s+|[(){}[\];,=:!|]|\.)/g);
  const tokens: string[] = [];
  for (let i = 0; i < rawTokens.length; i++) {
    ...
    if (
      rawTokens[i + 1] === '.' &&
      /^[A-Za-z_$][\w$]*$/.test(t) &&
      /^[A-Za-z_$][\w$]*$/.test(rawTokens[i + 2] ?? '')
    ) {
      tokens.push(`${t}.${rawTokens[i + 2]}`);
      i += 2;
    } else {
      tokens.push(t);
    }
  ...
  return tokens.map((tok, idx) => {
    if (!tok) return null;
    if (HIGHLIGHT_TOKENS.has(tok) || HIGHLIGHT_TOKENS.has(tok.split('.')[0])) {
      return (

      ...
      <div className="relative grid grid-cols-1 lg:grid-cols-[minmax(0,_0.85fr)_minmax(0,_1.4fr)] gap-lg items-center">
        ...
        <div className="flex flex-col gap-md min-w-0">
          ...
          <ul className="flex flex-wrap items-center gap-x-2 gap-y-1.5 pt-1">
            {content.flowLine.map((step, idx) => (
              <li key={step} className="flex items-center gap-2">
                <span
                  className={cn(
                    'inline-flex items-center rounded-full border px-2.5 py-0.5',
                    'border-sky-300/30 bg-white/5 text-xxsm font-mono text-sky-200',
                  )}
                >
                  {step}
                </span>
                {idx < content.flowLine.length - 1 && (
                  <ArrowRightIcon aria-hidden="true" className="h-3.5 w-3.5 text-sky-300/60" />
                )}
              </li>
            ))}
          </ul>

          <p className="text-xsm sm:text-sm leading-relaxed text-slate-300 break-keep max-w-[58ch]">
            {content.sub}
          </p>
        </div>
      </div>

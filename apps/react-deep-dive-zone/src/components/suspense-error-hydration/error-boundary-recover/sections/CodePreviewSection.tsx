...
type Props = { content: ErrorBoundaryRecoverContent['code'] };
      ...
      >
        ...
        <ol className="flex flex-col gap-3">
          {content.explanation.map((item, i) => (
            <li
              key={item.title}
              className="flex items-start gap-2.5 text-xsm text-[var(--term-fg)] break-keep"
            >
              <span
                aria-hidden="true"
              ...
              <div className="flex flex-col gap-0.5">
                <span className="text-sm font-bold text-blue-700 dark:text-blue-300 break-keep">
                  {item.title}
                </span>
                <span className="text-[var(--term-muted)] break-keep">{item.body}</span>
              </div>
            </li>
          ))}
        </ol>

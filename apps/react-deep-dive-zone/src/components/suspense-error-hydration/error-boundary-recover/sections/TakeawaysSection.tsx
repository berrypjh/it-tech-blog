...
type Props = { content: ErrorBoundaryRecoverContent['takeaways'] };
...
const toneClass = {
  ...
  rose: {
    card: 'border-rose-200/80 bg-rose-50/40 dark:border-rose-800/60 dark:bg-rose-950/20',
...
const cardIcons = [ShieldCheckIcon, RefreshCcwIcon, ShieldAlertIcon];
  ...
  <section aria-labelledby="takeaways-heading" className="flex flex-col gap-md">
    ...
    <ul className="grid grid-cols-1 gap-md md:grid-cols-3">
      {content.cards.map((card, i) => {
        ...
        const Icon = cardIcons[i] ?? ShieldCheckIcon;
        return (
          <li key={card.number}>
            <article
              className={cn(

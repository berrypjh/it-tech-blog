...
const renderToken = (tok: string, i: number) => {
  ...
  if (
    tok === 'claimNextHydratableSuspenseInstance' ||
    tok === 'tryHydrateSuspense' ||
    tok === 'throwOnHydrationMismatch' ||
    tok === 'markSuspenseBoundaryShouldCapture'
  )
    return (
  ...
  if (
    tok === 'fiber' ||
    tok === 'nextInstance' ||
    tok === 'suspenseInstance' ||
    tok === 'hydrationBoundary' ||
    tok === 'returnFiber' ||
    tok === 'sourceFiber' ||
    tok === 'root' ||
    tok === 'rootRenderLanes' ||
  ...
  if (tok === 'ForceClientRender')
    return (
      <span key={i} className="text-rose-300 font-bold">
        {tok}
      </span>
    );
  if (tok === 'div' || tok === 'span' || tok === 'p')
    return (

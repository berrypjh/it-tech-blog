...
export const WorkLoopCodeCheckpoint = ({ content }: Props) => (
  ...
  >
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,_0.32fr)_minmax(0,_0.68fr)] gap-md lg:gap-lg items-stretch">
      {/* Left file card */}
      <article
        className={cn(
      ...
      {/* Right: code panel + callouts */}
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,_1.55fr)_minmax(0,_1fr)] gap-md">
        <CodePanel
...
const Callout = ({ callout }: { callout: CodeCallout }) => {
  const isSync = callout.kind === 'sync';
  return (
    <aside
      className={cn(
        'flex flex-col gap-2 rounded-2xl border-2 border-dashed p-md',
        isSync
          ? 'border-sky-300/80 bg-sky-50/60 dark:border-sky-700/70 dark:bg-sky-950/25'
    ...
    >
      ...
      <p
        className={cn(
          'text-xsm sm:text-sm leading-snug font-bold break-keep',
          isSync ? 'text-sky-900 dark:text-sky-100' : 'text-teal-900 dark:text-teal-100',
        )}
      >
      ...
      <div className="flex items-baseline gap-2">
        <span
          className={cn(
            ...
            isSync ? 'text-sky-700 dark:text-sky-300' : 'text-teal-700 dark:text-teal-300',
          )}
        >
        ...
        <span
          className={cn(
            'flex-1 text-xsm sm:text-sm leading-snug break-keep',
            isSync ? 'text-sky-800 dark:text-sky-200' : 'text-teal-800 dark:text-teal-200',
          )}
        >
          {callout.meaningBody}
        </span>

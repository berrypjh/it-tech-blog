...
import type { CodeCallout, FunctionComponentContent } from '../content';
import { CodeIcon, FileCodeIcon, HelpCircleIcon } from '../icons';

import { tonePalette } from './tone-palette';

type Props = { content: FunctionComponentContent['code'] };

export const FunctionComponentCodeCheckpoint = ({ content }: Props) => (
  <section
...
const CodePanel = ({ title, subtitle, code, highlightLines }: CodePanelProps) => {
          ...
          {lines.map((line, idx) => {
            ...
            return (
              <div
                ...
                className={cn(
                  'flex',
                  isHighlight && 'bg-sky-100/70 dark:bg-sky-950/40 border-l-2 border-sky-500',
                )}
              >
                <span
                  aria-hidden="true"
    ...
    >
      <header className="flex items-center justify-between gap-2">
        <span
          aria-hidden="true"
          className={cn(
            'inline-flex h-7 w-7 items-center justify-center rounded-lg border font-mono font-bold text-xsm',
            palette.chip,
          )}
        >
          {callout.number}

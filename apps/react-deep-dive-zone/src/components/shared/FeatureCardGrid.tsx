...
export const FeatureCardGrid = ({ items, className }: Props) => (
  <ul
    className={cx('grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-md items-stretch', className)}
  >
    {items.map((item) => (
...
const FeatureCard = ({ item }: { item: FeatureCardItem }) => {
  ...
  return (
    ...
    >
      ...
      <h3 className={cx('text-sm font-bold tracking-tight break-keep', t.text)}>{item.title}</h3>

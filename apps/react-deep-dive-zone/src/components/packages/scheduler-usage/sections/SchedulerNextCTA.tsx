...
import { SharedLayerIllustration } from '../components/SharedLayerIllustration';
  ...
  return (
    <section aria-labelledby="heading-next" className="space-y-md">
      ...
      >
        <span
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_18%_28%,rgba(167,139,250,0.22),transparent_60%),radial-gradient(circle_at_82%_72%,rgba(251,191,36,0.18),transparent_55%)]"
        />
        <span
          aria-hidden="true"
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-[auto_minmax(0,1fr)_auto] gap-md sm:gap-lg items-center">
          <div className="flex justify-center lg:justify-start">
            <SharedLayerIllustration className="h-28 w-32 sm:h-32 sm:w-36" />
          </div>

          <div className="flex flex-col gap-2 min-w-0 text-center lg:text-left">
            <span className="text-[10px] uppercase tracking-wider text-sky-300 font-bold font-mono">
              {content.eyebrow}
            </span>
            <h2
              id="heading-next"
            ...
            <p className="text-sm sm:text-md leading-relaxed break-keep text-slate-200">
              <span>{content.line1Before}</span>
              <span className="font-bold font-mono text-cyan-300">{content.line1Accent}</span>
              <span>{content.line1After}</span>
            </p>
            <p className="text-sm sm:text-md leading-relaxed break-keep text-slate-200">
          ...
          <div className="flex flex-col gap-2 w-full lg:w-auto">
            <Link
              ...
              className={cn(
                ...
                'bg-violet-500 text-slate-950 text-xsm font-bold',
